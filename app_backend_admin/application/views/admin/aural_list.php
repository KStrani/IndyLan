			...
			<div id="content">
				...
				<section>
					<div class="section-header">
						<ol class="breadcrumb">
							<li><a href="#">Home</a></li>
							<li class="active">Aural Comprehension List</li>
						</ol>
					...
					<div class="section-body">
						...
						<div class="card">
							<div class="card-body">
								...
								<div  class="isa_success"><span><?php echo $success_msg; ?></span></div>
								<?php } ?>
								<?php 
								if(isset($error_msg)){ ?>
								<div class="isa_error"><span><?php echo $error_msg; ?></span></div>
								<?php } ?>
								
								<form action="<?php echo base_url(); ?>admin_master/aural_import" method="post" name="upload_excel" enctype="multipart/form-data" onsubmit="return checkvalid();">
									<div class="form-group row">
										...
										<div class="col-md-2 col-sm-3 col-xs-5 text-right mrgt5 pdr0">
											<a href="<?php echo base_url();?>admin_master/download_aural_sample" class="btn btn-primary">Sample Excel File</a>
										</div>
											<div class="col-md-1 col-sm-3 col-xs-5 mrgt5">
											<a href="<?php echo base_url();?>admin_master/excel_export_aural/<?php echo $category_select; ?>/<?php echo $subcategory_select; ?>" class="btn btn-primary">Export</a>
										</div>
									</div>
								</form>
								<form action="<?php echo base_url(); ?>admin_master/aural_list" method="post" name="upload_excel" enctype="multipart/form-data">
									<div class="form-group row">
								<div class="col-md-3 col-sm-3 col-xs-5 text-right mrgt5 pdr0">
									<input type="text" name="search_text" class="form-control" value="<?php  if(isset($search)) { echo $search; } ?>"/>
								</div>
								<div class="col-md-1 col-sm-3 col-xs-5 text-right mrgt5 pdr0">
									<button type="submit" class="btn btn-primary btn-raised" id="search_btn" name="search_btn">Search</button>
								</div>
								</div>
								...
								<!-- BEGIN DATATABLE 2 -->
								<form action="<?php echo base_url(); ?>admin_master/delete_all_aurals" method="post" onsubmit="return confirm_delete();">
									<div class="row">
										<h4 class="col-md-12 col-sm-12 mrgt15">Aural Comprehension List</h4>
										entries
										<div class="col-md-1">
										...
										<div class="col-md-10 col-sm-9 mrgt10 pull-right">
											<a href="<?php echo base_url(); ?>admin_master/add_aural" class="btn btn-primary btn-sm btn-raised pull-right">Add New</a>
											<input type="submit" class="btn btn-primary btn-sm btn-raised pull-right mrgr10" value="Delete Selected" name="submit"/>
											<input type="submit" class="btn btn-primary btn-sm btn-raised pull-right mrgr10" value="Delete Audios" name="submit"/>
											...
											<div class="table-responsive no-margin">
												...
												<table id="example" class="table order-column hover">
													...
													<tbody >
														...
														foreach ($aural_list as $key) {  //http://blue.alphademo.in/sfiapp/uploads/audio/katt_sw.m4a ?>
														<tr>
															<td><label class="checkbox-inline checkbox-styled"><input type="checkbox" name="delete[]" class="checkBoxClass" value="<?php echo $key['aural_id']; ?>"/><span></span></label></td>
															<td><a href="<?php echo base_url();?>admin_master/edit_aural/<?php echo $key['aural_id']; ?>"><i class="fa fa-pencil action"></i> </a>  <a onclick="return confirm('Are you sure?');" href="<?php echo base_url();?>admin_master/delete_aural/<?php echo $key['aural_id']; ?>"> <i class="fa fa-trash-o action"></i> </a></td>
															...
															<td><?php echo $key[$langkey['field_name']]; ?>
																...
																<?php if(!empty($aname) && file_exists($fileaudio) && $key['is_audio_available'] == '1' && $langkey['source_language_id']== $target_lang_id) { ?>
																...
																<?php }else if($key['is_audio_available']=="0" && $langkey['source_language_id']== $target_lang_id){ ?>

																<?php  }else if($langkey['source_language_id']== $target_lang_id){ ?>
...
<script type="text/javascript">
	...
	var per_page= $('#per_page').val();
		...
		$.ajax({
					...
					success:function(data){
						...
						$().redirect('<?php echo base_url();?>admin_master/aural_list', {'mode_id': modeid,'sort':sort,'per_page':per_page});

					//	window.location.href = '<?php echo base_url();?>admin_master/aural_list/'+modeid;
					},
		});
	...
	var per_page= $('#per_page').val();
		...
		$.ajax({
					...
					url:'<?php echo base_url();?>admin_master/aural_list/'+mode+'/'+category+'/'+subcate,
					success:function(data){
						
						//window.location.href = '<?php echo base_url();?>admin_master/aural_list/'+mode+'/'+category+'/'+subcate;
						$().redirect('<?php echo base_url();?>admin_master/aural_list', {'per_page':per_page,'mode_id': mode,'cate_id':category,'subcate_id':subcate,'sort':sort});
...
// ---------------------------------Filter Without Page refresh
			...
			// 					//window.location.href = '<?php echo base_url();?>admin_master/aural_list/'+mode+'/'+category+'/'+subcate;
			// 					//$().redirect('<?php echo base_url();?>admin_master/aural_list', {'mode_id': mode,'cate_id':category,'subcate_id':subcate});
	...
	var per_page= $('#per_page').val();
			...
			$.ajax({
					...
					success:function(data){
						...
						//window.location.href = '<?php echo base_url();?>admin_master/aural_list/'+mode+'/'+cate_id+'/'+subcate;
						$().redirect('<?php echo base_url();?>admin_master/aural_list', {'per_page':per_page,'mode_id': mode,'cate_id':cate_id,'subcate_id':subcate,'sort':sort});
					},			
			}); 
	...
	var cate_id= $('#category').val();
			...
			$.ajax({
					url:'<?php echo base_url();?>admin_master/aural_list/',
					...
					//	$('#subcate').find('option').remove().end().append(data);
						//window.location.href = '<?php echo base_url();?>admin_master/aural_list/'+mode+'/'+cate_id+'/'+subcate;
						$().redirect('<?php echo base_url();?>admin_master/aural_list', {'mode_id': mode,'cate_id':cate_id,'subcate_id':subcate,'sort':sort});
					},			
			}); 
	...
	var sort= $('#sort').val();
			...
			$.ajax({
					...
					//	$('#subcate').find('option').remove().end().append(data);
						//window.location.href = '<?php echo base_url();?>admin_master/aural_list/'+mode+'/'+cate_id+'/'+subcate;
						$().redirect('<?php echo base_url();?>admin_master/aural_list', {'mode_id': mode,'cate_id':cate_id,'subcate_id':subcate,'sort':sort,'per_page':per_page});
					},			
			}); 
